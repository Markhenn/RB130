...
# Problem
# update bubble sort to take a block to define sorting
# input array
# output nil -> mutates input array
#
# change the next if part
#  next if yield(a-1, a)
#
#  but have a default
...
arrau = %w(sue Pete alice Tyler rachel Kim bonnie)
...
def bubble_sort!(array)
  loop do
    ...
    1.upto(array.size - 1) do |index|
      a, b = array[index - 1], array[index]
      next if block_given? ? yield(a) <= yield(b) : a <= b
      array[index - 1], array[index] = array[index], array[index - 1]
      swapped = true
    end
